    ...
    def Refrash(self):
        ...
        for shop in self.Shops:
            if self.User_Shopes_Combobox.get() == shop['Shop_name'] or self.User_Shopes_Combobox.get() == "":
                url = shop['Shop_link']
                entry = {'Do': "Testing Connection", 'shop': shop, 'user' : self.user, 'ISNEW_USER' : 0 , 'ISNEW_SHOP' : 0 }
                while 1:
                    try:
                        # Send the data to the API
                        print('Data  ', entry)
                        ...
                        if response.status_code == 200:
                            ...
                            if not response_data == []:
                                if response_data['status'] == 'success':
                                    if shop['Shop_online_id'] == "" or shop['Shop_online_id'] == "None" or not shop['Shop_online_id']:
                                        entry = {'Do': "GET USER", 'shop': shop, 'user' : self.user, 'ISNEW_USER' : 1 , 'ISNEW_SHOP' : 1 }
                                        answer = tk.messagebox.askquestion("Question", "User Data is Not Uploded to Cloud Would. You Like To Start Uploding Them?")
                                        if answer == 'yes':
                                            continue
                                        else:
                                            self.found_linke_list_box.insert('', 'end', text="Erorr User Data is Not Uploded to Cloud Would.", values=(shop['Shop_name'], shop['Shop_link']))

                                    if shop['Shop_online_id'] == "" or shop['Shop_online_id'] == "None" or not shop['Shop_online_id']:
                                        entry = {'Do': "GET SHOP", 'shop': shop, 'user' : self.user, 'ISNEW_USER' : 1 , 'ISNEW_SHOP' : 1 }
                                        answer = tk.messagebox.askquestion("Question", "Shop Data is Not Uploded to Cloud Would. You Like To Start Uploding Them?")
                                        if answer == 'yes':
                                            continue
                                        else:
                                            self.found_linke_list_box.insert('', 'end', text="Erorr Shop Data is Not Uploded to Cloud Would.", values=(shop['Shop_name'], shop['Shop_link']))
