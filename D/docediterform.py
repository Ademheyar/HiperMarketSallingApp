...
class DocEditForm(tk.Frame):
    def __init__(self, master, items, id):
        ...
        self.list_items = ttk.Treeview(items_tab, columns=("Shop Name", "Color", "Size", "Barcode", "Qtyfirst", "Qty", "cdate", "update"))
        self.list_items.grid(row=1, column=0, columnspan=4, sticky="nsew")
        self.list_items.heading("#0", text="Shop Name", anchor=tk.W)
        self.list_items.column("#0", stretch=tk.NO, minwidth=25, width=125)
        self.list_items.heading("#1", text="Color", anchor=tk.W)
        self.list_items.column("#1", stretch=tk.NO, minwidth=25, width=125)
        self.list_items.heading("#2", text="Size", anchor=tk.W)
        self.list_items.column("#2", stretch=tk.NO, minwidth=25, width=125)
        self.list_items.heading("#3", text="Barcode", anchor=tk.W)
        self.list_items.column("#3", stretch=tk.NO, minwidth=25, width=125)
        self.list_items.heading("#4", text="Qtyfirst", anchor=tk.W)
        self.list_items.column("#4", stretch=tk.NO, minwidth=25, width=125)
        self.list_items.heading("#5", text="Qty", anchor=tk.W)
        self.list_items.column("#5", stretch=tk.NO, minwidth=25, width=125)
        self.list_items.heading("#6", text="cdate", anchor=tk.W)
        self.list_items.column("#6", stretch=tk.NO, minwidth=25, width=125)
        self.list_items.heading("#7", text="update", anchor=tk.W)
        self.list_items.column("#7", stretch=tk.NO, minwidth=25, width=125)
        ...
        if "|)," in items_:
            ...
            for values in items_lists:
                ...
                if len(item) == 10:
                    ...
                    TAX += float(tax)
                    print("list | " + str([code, name, shop, color, size, qty, price, total_price, PRICE, disc, Disc, tax, TAX]))
                    self.list_items.insert("", 'end', text="", values=(code, name, shop, color, size, qty, price, total_price, PRICE, disc, Disc, tax, TAX))
        elif ":)," in items_:
            print("spliting by :")
            items_lists = items_.split(":),")
            for values in items_lists:
    ...
    def save_change(self):
        ...
        # TODO: reduce qty if it i new item
