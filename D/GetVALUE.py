...
class GetvalueForm(tk.Tk):
    def __init__(self, master, txt, title):
        ...
        if title != None and len(title) > 0:
            self.getvalue_form.title(title[self.onvalue])
        else:
            pass
        ...
        self.display_hold_and_opration = tk.Entry(self.getvalue_form, text="", font=("Arial", 15), justify="left")
        self.display_hold_and_opration.grid(row=0, column=0, sticky="nsew", columnspan=4)
        ...
        self.get_amount_entry.grid(row=1, column=0, sticky="nsew", columnspan=4)
    ...
    def add_num(self, event, text):
        ...
        elif text == "equal" or text == "minus" or text == "plus" or text == "asterisk" or text == "slash":
            # Do nothing if the button is "+", "-", "*", or "/"
            if self.hold_value != 0.0 and self.opration != "" and self.include_var.get() != "":
                current_value = str(eval(str(self.hold_value) + self.opration + self.include_var.get()))
                self.include_var.set(current_value)
                self.opration = ""

            if text == "minus":
                self.opration = "-"
            elif text == "plus" or text == "equal":
                self.opration = "+"
            elif text == "asterisk":
                self.opration = "*"
            elif text == "slash":
                self.opration = "/"
            if current_value != "":
                self.hold_value = float(current_value)
            self.display_hold_and_opration.delete(0, 'end')
            self.display_hold_and_opration.insert(0, str(self.hold_value) + " " + self.opration)
            self.include_var.set("")
        elif text == "":
            # Do nothing if the button is "+", "-", "0", "." 
            pass
        elif text in ["Return", "KP_Enter", "enter"]:
            # "enter" or "Return" or "KP_Enter" key is pressed
            if self.hold_value != 0.0 and self.opration != "" and self.include_var.get() != "":
                current_value = str(eval(str(self.hold_value) + self.opration + self.include_var.get()))
                self.include_var.set(current_value)
                self.opration = ""
                ...
                try:
                    ...
                    if current_value == "":
                        self.value.append(self.hold_value)
                    else:
                        self.value.append(float(current_value))
                    # Call the give_value method to set self.vv and destroy the Toplevel window
                    
                    if len(self.title) > self.onvalue+1:
                        self.onvalue += 1 
                        self.getvalue_form.title(self.title[self.onvalue])
                        self.hold_value = 0
                        self.include_var.set("")
                        current_value = ""
                        self.opration = ""
