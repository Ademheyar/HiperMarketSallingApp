    ...
    if "|)," in items_text:
        ...
        for values in items_lists:
            ...
            if len(item) >= 9:
                ...
                list_items.append([id, code, "", name, shop, color, size, qty, price, total_price, PRICE, disc, Disc, tax, TAX])
    else:
        print("spliting by :")
        items_lists = items_text.split(":),")
        for values in items_lists:
...
def load_payment(payment_text):
    #print("tiems : " + str(payment_text))
    list_items = []
    if ")" in payment_text or ")," in payment_text:
        items_lists = (payment_text + ",").split("),")
        index = 0
        for p in range(len(items_lists)-1):
            ...
            list_items.append([pay_id, pay_type, pay_pid, pay_pid_date, pay_updated_date, pay_user])
            index += 1
                
    elif "," in payment_text:
        items_lists = payment_text.split(",")
        index = 0
        for p in range(len(items_lists)-1):
            item = items_lists[p].split(" = ")
            ...
            price = item[1]
            #print("list : " + str([name, price]))
            list_items.append([index, name, price, self.created_date, self.created_date, self.created_user])
            index += 1
    else:
        item = payment_text.split(" = ")
        print("item :" + str(item))
        name = item[0].replace("(:", "")
        price = item[1]
        list_items.append(["0", name, price, self.created_date, self.created_date, self.created_user])
    return list_items
...
'''class DocEditForm(tk.Frame):
    def __init__(self, master, items, id):
        ...
        rows = Update_table_database("SELECT * FROM users")
    ...
    def save_change(self):
        ...
        for a in self.list_items.get_children():
            ...
            id = i['text']
            print(str(id))
            Update_table_database("SELECT * FROM product WHERE id=?", (id,))
            it = cursor.fetchone()
            if it:
                item += "(:"
                item += str(id) # ID
        ...
        for olditem in self.olditems:
            # [id, code, name, shop, color, size, qty, price]
            Update_table_database("SELECT * FROM product WHERE id=?", (olditem[0],))
            it = cursor.fetchone()
            if len(it) > 0:
        ...
        for a in self.list_payment.get_children():
            ...
            iv = p['values']
            id = p['text']
            Update_table_database("SELECT * FROM tools WHERE name=?", (iv[0],))
            rows = cursor.fetchall()
            print("rows:" + str(rows))
