...
def Update_Shop(Link, userARG, userVALUE, find_Arg, find_Value):
    query = ""
    value = None
    fquery = ""
    fvalue = None
    ...
    for a, arg in  enumerate(userARG):
        if len(userVALUE) > a:
            query += " " + arg + "=?"
            if value:
    ...
    if Link and islinked(Link):
        url = Link
        entry = {'Do': "UPDATE USER", 'QUERYS': query, 'QUERYVALUES' : value,  'FIND_ARG': fquery, 'FIND_VALUE': fvalue}
        response_data = Sand_API(url, entry)
